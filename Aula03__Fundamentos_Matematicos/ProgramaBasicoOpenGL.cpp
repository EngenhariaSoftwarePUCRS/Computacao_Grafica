...
/*void moveMa(Poligono P) {
    PosFoguete.x++;
    for (int i = 0; i < P.getNVertices(); i++) {
        P.alteraVertice(i, PosFoguete);
        PosFoguete.y = (i % 2 == 0) ? 1 : 0;
    }
}*/
...
void keyboard ( unsigned char key, int x, int y )
{
	...
	switch ( key )
	{
	    case 'a':
            PosFoguete.x--;
            P1.alteraVertice(0, PosFoguete);
            PosFoguete.y = 1;
            P1.alteraVertice(1, PosFoguete);
            PosFoguete.y = 0;
            P1.alteraVertice(2, PosFoguete);
            PosFoguete.y = 1;
            P1.alteraVertice(3, PosFoguete);
            PosFoguete.y = 0;
            break;
        case 'w':
            PosFoguete.y++;
